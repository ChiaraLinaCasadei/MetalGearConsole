...
public class MisionIntermedia extends Mision {
    ...
    public Boolean Iniciar() {
        ...
        while (juegoActivo) {
            ...
            switch (entrada) {
                case "w":
                    movResp = mapa.snake.Mover("arriba", mapa.mapa);
                    break;
                case "s":
                    movResp = mapa.snake.Mover("abajo", mapa.mapa);
                    break;
                case "a":
                    movResp = mapa.snake.Mover("izquierda", mapa.mapa);
                    break;
                case "d":
                    movResp = mapa.snake.Mover("derecha", mapa.mapa);
                    break;
                default:
    ...
    public Boolean EsVictoria() {
        ...
        var objetivo = mision == 1 ? mapa.hangar : mapa.puertaBloqueada;
        ...
        if (mision == 1) {
            if (mapa.EnPerimetroSnake(objetivo.posicion, 1) && mapa.snake.hasCrucialItem) {
                return true;
            }
        } else {
            var enPuertaConExplosivo = mapa.EnPerimetroSnake(objetivo.posicion, 1) && mapa.snake.hasCrucialItem;
    ...
    private Boolean GuardiasCerca() {
        for (int i = 0; i < mapa.cantidadGuardias; i++) {
            var posicionGuardia = mapa.ubicacionCeldaGuardias[i].posicion;
            if (mapa.EnPerimetroSnake(posicionGuardia, 2)) {
                return true;
            }
        }
        return false;
